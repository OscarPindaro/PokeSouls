	...
	for i in range(vframes):
		for j in range(hframes):
			var red_pos : Vector2 = get_color_position(
				image, RED, frame_heigth*i, frame_heigth*(i+1),
				frame_width*j, frame_width*(j+1))
			var blue_pos : Vector2 = get_color_position(
				image, BLUE, frame_heigth*i, frame_heigth*(i+1),
				 frame_width*j, frame_width*(j+1))
			var green_pos : Vector2 = get_color_position(
				image, GREEN, frame_heigth*i, frame_heigth*(i+1),
				 frame_width*j, frame_width*(j+1))
			var black_pos : Vector2 = get_color_position(
				image, BLACK, frame_heigth*i, frame_heigth*(i+1),
				 frame_width*j, frame_width*(j+1))
...
func get_color_position(image : Image, color : Color, start_row : int, end_row : int, start_col : int, end_col : int ) -> Vector2:
	image.lock()
